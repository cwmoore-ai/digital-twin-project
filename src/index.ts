...
class DigitalTwinServer {
  ...
  private getTasks(): any[] {
    ...
    const tasks = allItems.filter((item: memoryStore.MemoryItem) => item.type === 'task');
    return tasks;
  }
  ...
  private handleToolCall(message: any) {
    ...
    // Handle memory-related tools
      ...
      try {
        ...
        this.sendError(message.id, `Error handling memory tool: ${(error as Error).message}`);
        return;
      }
    }
  ...
  private handleMemoryTool(name: string, args: any) {
    switch (name) {
      case 'memory_get_all':
        const allItems = memoryStore.getAllMemoryItems();
